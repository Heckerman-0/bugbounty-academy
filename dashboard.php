...
// Route by slug so new content always maps to the right page
                ...
                } elseif ($slug == 'owasp-challenge-lab') {
                    $link = 'modules/labs/owasp_lab.php?id=' . (int)$item['id'];
                } elseif ($slug == 'dir-traversal-lab') {
            ...
            <?php
                // --- QUIZ for this module: shown after lesson+lab, locked until both done ---
                $quizInfo = getModuleQuiz($group);
                if ($quizInfo):
                    $quizUnlocked = isModuleLessonLabComplete($_SESSION['user_id'], $group);
                    $quizProgress = getProgress($_SESSION['user_id']);
                    // Quiz "complete" if the linked content item is marked done
                    $quizDone = isset($quizProgress[$quizInfo['content_id']]) && $quizProgress[$quizInfo['content_id']] == 'completed';
                    $quizIcon = $quizDone ? '✅' : '📝';
                    $quizStyle = $quizUnlocked ? '' : 'opacity:0.55;';
            ?>
                <div class="module-item" style="<?= $quizStyle ?>">
                    <span><?= $quizIcon ?></span>
                    <?php if ($quizUnlocked): ?>
                        <a href="<?= BASE_URL ?>modules/quizzes/take.php?quiz_id=<?= (int)$quizInfo['quiz_id'] ?>"><?= htmlspecialchars($quizInfo['quiz_title']) ?></a>
                    <?php else: ?>
                        <span style="cursor:not-allowed; color:#888;">🔒 <?= htmlspecialchars($quizInfo['quiz_title']) ?></span>
                    <?php endif; ?>
                    <span class="status"><?= $quizDone ? '🏆' : '🔒' ?></span>
                </div>
            <?php endif; ?>
        </div>
    </div>
